      ...
      <div className="grid h-16 grid-cols-[1fr_auto_1fr] items-center border-b border-border bg-background/85 px-4 backdrop-blur-md md:h-20 md:px-6 lg:px-10">
        ...
        <nav className="flex items-center">
          <Sheet>
            ...
            <SheetContent side="left" className="flex w-80 flex-col gap-0 bg-card p-0 sm:max-w-sm">
          ...
          <div className="ml-2 hidden items-center gap-7 lg:flex">
            ...
            {collections.map(c => <Link key={c.slug} to={`/collections/${c.slug}`} className={navLink}>
                {c.name}
              </Link>)}
          </div>
        </nav>
        ...
        {/* Center: wordmark */}
        <Link to="/" className="justify-self-center font-display text-2xl tracking-tight md:text-3xl">
          Noewe
        </Link>

        {/* Right: cart */}
        <div className="flex items-center justify-end">
          <button type="button" onClick={openCart} aria-label={t("header.cart")} className="relative flex h-10 w-10 items-center justify-center">
            <ShoppingBag className="h-5 w-5" />
            {itemCount > 0 && <span className="absolute right-0 top-0 flex h-4 min-w-4 items-center justify-center rounded-full bg-foreground px-1 text-[10px] font-medium text-background">
                {itemCount}
              </span>}
          </button>
        </div>
